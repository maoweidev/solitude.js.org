  ...
  <div>
    <ULandingSection class="!pt-0 from-gray-950/50 to-gray-900">
      <ULandingCTA align="left" card :ui="{
        ...
        <template #links>
          <ClientOnly>
            <UAvatarGroup :max="16" size="md" class="flex-wrap-reverse [&_span:first-child]:text-xs justify-center">
              <UTooltip v-for="(contributor, index) of contributors" :key="index" :text="contributor.login"
                class="rounded-full" :ui="{ background: 'bg-gray-50 dark:bg-gray-800/50' }"
                :popper="{ offsetDistance: 16 }">
                <UAvatar :alt="contributor.login" :src="`https://ipx.nuxt.com/s_40x40/gh_avatar/${contributor.login}`"
                  ...
                  <NuxtLink :to="`https://github.com/${contributor.login}`" :aria-label="contributor.login"
